...
const handleMessage = async (data: any) => {
  ...
  switch (data.type) {
    case 'mesherData': {
      ...
      // Replay anything that arrived before WASM was ready, in order.
      if (pendingUntilReady.length > 0) {
        const q = pendingUntilReady
        pendingUntilReady = []
        console.log('[WASM Mesher] draining', q.length, 'messages queued before WASM was ready')
        for (const d of q) void handleMessage(d)
      }
      break
    }
    case 'dirty': {
