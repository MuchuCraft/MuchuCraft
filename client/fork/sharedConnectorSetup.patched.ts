...
export function buildItemMapper (version: string) {
  ...
  return (raw: { type: number; count: number; metadata?: number; nbt?: unknown; components?: any[] },
    ...
    try {
      ...
      // Same guard for lore lines carried over from the base mapping.
      const cleanLore = Array.isArray((mapped as any).lore)
        ? (mapped as any).lore.filter((l: any) => !looksRaw(typeof l === 'string' ? l : JSON.stringify(l)))
        : (mapped as any).lore;
